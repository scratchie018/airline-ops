...
export default function AircraftPage() {
  ...
  async function retire(id: string) {
    if (!confirm("Retire this aircraft? It'll be hidden from new flight scheduling.")) return;
    await apiFetch(`/aircraft/${id}`, { method: "DELETE" });
    load();
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold"><i className="fa-solid fa-warehouse text-brand-400 mr-2" />Fleet</h1>
              ...
              <tr key={a.id}>
                ...
                {can(Permission.MANAGE_AIRCRAFT) && (
                  <td className="px-4 py-2 text-right">
                    {a.status !== "RETIRED" && (
                      <button onClick={() => retire(a.id)} className="text-tuired-400 hover:underline text-xs">
                        Retire
                      </button>
                    )}
                  </td>
